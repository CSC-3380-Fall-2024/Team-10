/// @description car_update(accelerate,steer_direction)
/// @param accelerate
...
function car_update(argument0, argument1) {
	...
	if kill_sideways_speed = 1 //kills the lateral velocity of the wheel so that the wheel sticks to the road and so that the car doesn't slide around like a bar of soap
	//see the inits of the various car types (eg. sportscar_init), the script 'kill_sideways_speed' and obj_car create 4
	with(wheel) 
	  kill_sideway_speed(other.drift); //the drift value ensures that the lateral velocity of the wheel isn't killed completely, and that there is some drift
	}
