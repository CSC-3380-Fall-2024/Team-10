function AG_ship_init() {
	...
	drift = 0.6; //drift control, usually between (0-1) but can be higher  //This value for the car (but not the wheel) has been disabled in the code because it causes the cars in collisions to have hardly any weight
	                        //0: no control | 1: high control
	drift_default = 0.6 //same number as above.  See step 2 of obj_car which returns the drift back to this default number after drifting                        
	drift_factor = 0.2 //see step 2 of obj_car, where this drift factor increases when holding left or right

	max_speed = 9.5 //MAX SPEED maximum speed car can attain 
	max_speed_rev = 8.5; //REVERSE maximum speed car can attain reversing

	//these values will largely depends on the density of the car
	engine_acc = 12; //ACCELERATION forward force, moving forward
	engine_brk = 6;  //BRAKING  braking force
	engine_rev = 3;  //reverse force, moving backward

	max_wheel_angle = 90;  //STEERING  the maximum angle the wheel can rotate
	wheel_angle_velocity=7; //STEERING how quickly a wheel rotates.  0 is hardly rotates
	...
	torque_acc = 9;  //lower value is slower turning, higher value is faster turning      //extra torque on the car when turning 
